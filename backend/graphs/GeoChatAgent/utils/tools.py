...
class DataAnalysisTool:
    ...
    @staticmethod
    def extract_data_with_coordinates(
        ...
        for feature in geojson["features"]:
            if feature.get("properties") and "DN" in feature["properties"]:
                ...
                if isinstance(dn_value, (int, float)) and dn_value > 0:
                    ...
                    elif feature.get("geometry") and feature["geometry"].get(
                        "coordinates"
                    ):
    ...
    def calculate_statistics(
        values: List[float], geojson: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_temporal_trends(
        ...
        for year in sorted(years):
            ...
            if geojson:
                ...
                yearly_stats[year] = DataAnalysisTool.calculate_statistics(
                    values, geojson
                )
        ...
        if len(yearly_stats) > 1:
            ...
            for i in range(1, len(sorted_years)):
                ...
                if (
                    yearly_stats[current_year]["center_of_mass"]["longitude"] != 0
                    and yearly_stats[prev_year]["center_of_mass"]["longitude"] != 0
                ):
                    # Calculate distance between centers of mass
                    prev_coords = (
                        yearly_stats[prev_year]["center_of_mass"]["longitude"],
                        yearly_stats[prev_year]["center_of_mass"]["latitude"],
                    )
                    curr_coords = (
                        yearly_stats[current_year]["center_of_mass"]["longitude"],
            ...
            if (
                yearly_stats[first_year]["mean"] > 0
                and yearly_stats[last_year]["mean"] > 0
            ):
                total_percent_change = (
                    (yearly_stats[last_year]["mean"] - yearly_stats[first_year]["mean"])
                    / yearly_stats[first_year]["mean"]
                    * 100
                )
                ...
                return {
                    ...
                    "average_yearly_change": round(
                        sum(changes.values()) / len(changes), 2
                    )
                    if changes
                    else 0,
                    "total_change": {
    ...
    def compare_regions(
        ...
        for country in countries:
            ...
            if geojson:
                ...
                regional_stats[country] = DataAnalysisTool.calculate_statistics(
                    values, geojson
                )
        ...
        if regional_stats:
            rankings = {
                ...
                "highest_median": max(
                    regional_stats.items(), key=lambda x: x[1]["median"]
                )[0],
                "lowest_median": min(
                    regional_stats.items(), key=lambda x: x[1]["median"]
                )[0],
            ...
            if len(centers_of_mass) > 1:
                ...
                northernmost = max(
                    centers_of_mass.items(), key=lambda x: x[1]["latitude"]
                )[0]
                southernmost = min(
                    centers_of_mass.items(), key=lambda x: x[1]["latitude"]
                )[0]
                ...
                easternmost = max(
                    centers_of_mass.items(), key=lambda x: x[1]["longitude"]
                )[0]
                westernmost = min(
                    centers_of_mass.items(), key=lambda x: x[1]["longitude"]
                )[0]
